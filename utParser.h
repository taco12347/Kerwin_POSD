...
TEST_F(ParserTest, ListAsStruct) {
    ...
    ASSERT_EQ(2, stru->arity());
    ASSERT_EQ(".(1, [])", stru->symbol());
    ASSERT_EQ("1", stru->args(0)->symbol());
    ASSERT_EQ("[]", stru->args(1)->symbol());
}
...
TEST_F(ParserTest, ListAsStruct2) {
    ...
    Struct* stru = dynamic_cast<Struct*>(term);
    ASSERT_EQ(2, stru->arity());
    ASSERT_EQ(".(2, .(1, []))", stru->symbol());
    ASSERT_EQ("2", stru->args(0)->symbol());
    ASSERT_EQ(".(1, [])", stru->args(1)->symbol());
}
